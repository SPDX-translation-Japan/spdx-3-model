...
识别许可证补充内容(`LicenseAddition`)的全文。

## Description @zh-Hans

附加文本（`additionText`）包含许可证补充内容(`LicenseAddition`)的纯文本，但不包含模板或其他类似的标记。

当为了匹配的目的将附加文本与另一个文本进行比较时，许可证附加文本（`additionText`）的用户可以应用 [SPDX许可证列表匹配指南](../../../annexes/license-matching-guidelines-and-templates.md)。
