...
指定被弃用的此许可证或例外标识符所使用的SPDX许可证列表版本。
 
## Description @zh-Hans

对于在[SPDX许可证列表](https://spdx.org/licenses/)上列出的许可证（`ListedLicense`），或者对于在[SPDX许可证例外](https://spdx.org/licenses/exceptions-index.html)上列出的许可证例外（`ListedLicenseException`），弃用版本（`deprecatedVersion`）指定了许可证列表中第一个被标记为已弃用的版本是哪个版本。
