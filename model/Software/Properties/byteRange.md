...
スニペット情報が適用される元のホストファイル内のバイト範囲を定義する。

## Description @jp

このフィールドは、スニペット情報が適用される元のホストファイル内のバイト範囲を定義する。

バイト範囲は、様々なフォーマットの懸念事項から独立で、差異を参照する最も正確な方法である。W3Cのポインターメソッド語彙と一貫性を保つため、バイト範囲の数値は1から始まるようにした。
