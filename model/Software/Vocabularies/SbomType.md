...
ツールが作成する可能性のあるSBOMの共通種別を記述するために使用される値のセットを提供する。

## Description @jp

2023年4月21日に発行された[Types of Software Bill of Material (SBOM) Documents](https://www.cisa.gov/sites/default/files/2023-04/sbom-types-document-508c.pdf)で定義された、定義とSBOM種別のセット 。

SBOM種別は生成者の観点から最も可能性の高いSBOM種別を記述することで、利用者がSBOM内のデータについての結論を導き出せる。

1つのSBOMは、複数のSBOMドキュメント種別を持つことができる。

## Entries @jp

- design: 新しいソフトウェアアーティファクトに含まれるコンポーネント(一部はまだ存在しない可能性がある)を含む、意図され計画されたソフトウェアプロジェクトまたは製品のSBOM。
- source: 開発環境、ソースファイル、および製品アーティファクトの構築に使用される依存関係から直接作成されたSBOM。
- build: ソースファイル、依存関係、ビルドされたコンポーネント、ビルドプロセスの一時データ、その他のSBOMなどのデータからリリース可能なアーティファクト(実行可能ファイルやパッケージなど)を生成するためにソフトウェアをビルドするプロセスの一部として生成されるSBOM。
- deployed: SBOMは、システム上に存在するソフトウェアのインベントリを提供する。これは、構成オプションの分析と、(シミュレートされた可能性のある)デプロイ環境での実行動作の検査を組み合わせた、他のSBOMの集合体かもしれない。
- runtime: ソフトウェアを実行しているシステムを計測(訳注: instrumenting)すること、システム内に存在するコンポーネントだけでなく、外部へ呼び出したり動的にロードされるコンポーネントの検出を含む、を通して生成されるSBOM。ある文脈では、「Instrumented」または「Dynamic」SBOMと呼ばれることもある。
- analyzed: ビルド後のアーティファクト(実行ファイル、パッケージ、コンテナー、仮想マシンイメージなど)の分析を通して生成されるSBOM。このような分析は、通常、様々なヒューリスティックスが必要である。ある文脈では、これは「3rd party」SBOMと呼ばれることもある。
