...
データセットの可用性の可能な種別を記述する。それは、データセットが直接ダウンロードできるか、データを収集するスクリプトを使用して集められるか、クリックか登録フォーム後のみ利用できるか、を示す。

## Entries @jp

- clickthrough: データセットは公開されておらず、Webページで利用規約に同意した場合のみアクセスできる。
- directDownload: データセットは公開されており、直接ダウンロードできる。
- query: データセットは公開されているが、一度に全てではなく、データセットの一部を返すクエリーを通してのみアクセスできる。
- registration: データセットは公開されておらず、データセットにアクセスする前にメールによる登録が必要であるが、利用規約に同意する必要は無い。
- scrapingScript: データセット提供者が基本的なデータを提供しておらず、データセットを再構成する必要がある。通常は、データ収集のために提供されているスクリプトを使用する。
