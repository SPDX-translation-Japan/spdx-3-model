SPDX-License-Identifier: Community-Spec-1.0

# SafetyRiskAssessmentType

## Summary

Specifies the safety risk level.

## Description

Lists the different general safety risk levels that can be used to describe the
general safety risk of an AI system.

Using categorization according to the
[EU general risk assessment methodology](https://ec.europa.eu/docsroom/documents/17107).
The methodology implements Article 20 of Regulation (EC) No 765/2008 and is
intended to assist authorities when they assess general product safety
compliance.

## Metadata

- name: SafetyRiskAssessmentType

## Entries

- serious: The highest level of risk posed by an AI system.
- high: The second-highest level of risk posed by an AI system.
- medium: The third-highest level of risk posed by an AI system.
- low: Low/no risk is posed by an AI system.

## Summary @zh-Hans

指定安全风险级别。

## Description @zh-Hans

列出可用于描述AI系统的一般安全风险的不同级别。

使用[欧盟一般风险评估方法](https://ec.europa.eu/docsroom/documents/17107)进行分类。
该方法落实了第765/2008号条例（EC）第20条，旨在帮助当局评估产品的一般安全合规性。

## Entries @zh-Hans

- serious: AI系统可能导致的最高级别风险。
- high: AI系统可能导致的次高级别风险。
- medium: AI系统可能导致的第三高风险。
- low: AI系统可能导致的低风险/没有风险。

## Summary @ja

安全リスクレベルを指定する。

## Description @ja

AIシステムの一般的な安全リスクを記述するために使用できる、さまざまな一般的な安全リスクレベルを列挙する。

[EU一般リスク評価手法](https://ec.europa.eu/docsroom/documents/17107) に従った分類を用いる。
この手法は、規則 (EC) No 765/2008 の第20条を実施するものであり、
当局が一般製品安全性の適合性を評価する際に支援することを目的としている。

## Entries @ja

- serious: AIシステムによってもたらされる最高レベルのリスク。
- high: AIシステムによってもたらされる2番目に高いレベルのリスク。
- medium: AIシステムによってもたらされる3番目に高いレベルのリスク。
- low: AIシステムによってもたらされる低レベルのリスク、もしくはリスクが存在しない状態。
