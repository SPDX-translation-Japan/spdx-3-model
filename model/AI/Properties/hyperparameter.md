...
AIパッケージに含まれるAIモデルの構築に使用されたハイパーパラメータを記録する。

## Description @ja

ハイパーパラメータの値を記録する。

ハイパーパラメータとは、学習アルゴリズムの挙動を制御するために、トレーニングプロセス開始前に設定されるものである。これに対し、モデルパラメータはトレーニング中にデータから学習されるものである。開発者は通常、ハイパーパラメータを手動で設定するか、ハイパーパラメータチューニング（試行錯誤とも呼ばれる）を通じて調整する。

ハイパーパラメータの例としては、学習率、バッチサイズ、ニューラルネットワークの層数などがある。
